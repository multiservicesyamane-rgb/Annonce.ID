...
export default function ContactActions({ phone, title, adId, sellerId, externalUrl, source, orderWhatsapp }: { phone?: string; title: string; adId?: string; sellerId?: string; externalUrl?: string | null; source?: string | null; orderWhatsapp?: string | null }) {
  ...
  // Commande par WhatsApp (produits affiliés) : le bouton ouvre TON WhatsApp,
  // le lien affilié reste privé. Prioritaire sur le lien externe.
  const orderWaNumber = (() => {
    if (!orderWhatsapp) return "";
    let clean = orderWhatsapp.replace(/[^0-9]/g, "");
    if (clean.length === 9 && (clean.startsWith("7") || clean.startsWith("3"))) clean = "221" + clean;
    return clean;
  })();
  if (orderWaNumber) {
    const waOrder = `https://wa.me/${orderWaNumber}?text=${encodeURIComponent(`Bonjour, je souhaite commander : "${title}". Est-il disponible ?`)}`;
    return (
      <>
        <div className="flex flex-col gap-3">
          <a href={waOrder} target="_blank" rel="noopener noreferrer" className="btn btn-wa w-full py-3.5 text-[1.05rem] font-bold flex items-center justify-center gap-2">
          ...
          <p className="text-center text-[.72rem] text-gray-500 dark:text-white/50">
            Cliquez pour commander directement. Paiement et livraison au Sénégal.
          </p>
        </div>
